...
- **Gotchas found while verifying visuals in the live theater:**
  ...
  - `setCamera()` / `frameCamera()` **turn Auto-cam off**. **Free** cameras (sun, earth, moon, booster, tower, chase) allow WASD / mouse; **Fixed** livestream mounts (aerial, starbase, trench, hull, engines, gridfin, fin, drone) reject movement (`frameCamera` is a no-op there). `setCamera("gridfin")` is the grid-fin mount, not booster-hull-cam. `setCamera("booster")` looks at Super Heavy from outside. `setCamera("tower")` looks at Mechazilla (WASD along the ground). `setCamera("chase")` is Starship. `setCamera("aerial")` is the pad flying drone (T− hold wide). `setCamera("drone")` is the sea-level recovery drone (post-splash orbit of the floating ship), not Ship chase. Webcast mounts (`boosterHull`, `engines`, `flap`) are applied by Auto-cam at shot times — press **G** to re-enable Auto-cam, then `seek()` to the shot (e.g. Flight 13 T+4:16 for booster hull; T+1:05:26 for the drone).

...
- Super Heavy V3 **look-reference** (not the runtime craft): Ijsz23 Sketchfab mesh, CC BY 4.0 — [`assets/sketchfab/`](assets/sketchfab/), credit in `docs/STARSHIP.md`. `npm run measure-sketchfab` walks named nodes in `scene.gltf` (accessor min/max + 4×4 matrices). Do **not** `GLTFLoader` the 3.8M-tri `scene.bin` into the live theater (author: render-only; gitignored). Booster-only — it does not inform ship flaps / TPS.

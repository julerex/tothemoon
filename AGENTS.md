...
- **Gotchas found while verifying visuals in the live theater:**
  ...
  - `setCamera()` / `frameCamera()` **turn Auto-cam off**. **Free** cameras (sun, earth, moon, booster, tower, chase) allow WASD / mouse; **Fixed** livestream mounts (aerial, ground1, tower1cam, tower2cam, starbase, trench, hull, payload, engines, gridfin, fin, drone) reject movement (`frameCamera` is a no-op there). `setCamera("gridfin")` is the grid-fin mount, not booster-hull-cam — the lattice fills the frame, so Auto-cam uses the `boosterHull` variant for the webcast grid-fin stills. `setCamera("booster")` looks at Super Heavy from outside. `setCamera("tower")` looks at Mechazilla (WASD along the ground). `setCamera("tower1cam")` is Tower One Cam (OLP-1 peak looking at the stack). `setCamera("tower2cam")` is Tower Two Cam (OLP-2 peak); Auto-cam **pans it with the stack** for the T+3 → T+8 tower-down cut (`towerTrack`). `setCamera("chase")` is Starship. `setCamera("aerial")` is the Launchpad Drone, which **flies** the pad-local path in `src/camera/padDrone.ts` (T− tableau → perch ~600 m over the pad at liftoff → tableau) and tracks the stack while it is within 6 km. `setCamera("ground1")` is Ground Camera One (T−2 full stack and tower). `setCamera("payload")` is the Pez-bay cam (Starlink deploy, T+16:46 → T+27:39). `setCamera("drone")` is the sea-level recovery drone (post-splash orbit of the floating ship), not Ship chase. Webcast mounts (`boosterHull`, `engines`, `flap`, `payload`) are applied by Auto-cam at shot times — press **G** to re-enable Auto-cam, then `seek()` to the shot (e.g. Flight 13 T+4:53 for booster hull; T+1:05:26 for the drone).
 - Flight 13 Auto-cam is a **still-by-still cut list** (`src/camera/webcastShots.ts`): one entry per camera the replay cut to, at the mission second the still caught the switch. Keep it that way — when a still and the table disagree, the still wins. The theater ascent climbs a little faster than the webcast HUD, so pad-camera heights / tilts are tuned against the theater profile, not the burned-in altitude.

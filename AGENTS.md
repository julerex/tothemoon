...
- **Gotchas found while verifying visuals in the live theater:**
  ...
  - **Flight 13 splash hover:** the published buoy (`FLIGHT13_SPLASH_LAT` 19°S 107°E, Flight 11 McDowell analog) is not where the ship meets the water (~20°S, ~240 km away). `splashFloatRadiusKm` is the geocentric radius **at that buoy**. Stamping it onto the ship's ray lifts the hull by hundreds of meters — WGS84 radius changes ~270 m per degree here — so Starship hangs above the sea. Seat with `splashSeatRadiusAlong(earthSurfaceRadiusAlong(ship ray), t, wave)`. `wave` is `oceanSwellHeightKm(0, 0, t) + oceanChopHeightKm(0, 0, t)` (chop-plate center once `seatSea` has followed the craft). `SPLASH_WATERLINE_ALT_KM` (0.0014 km) is the chop mesh's local Y; keep them the same. `seatSea(craftWorld)` moves only the sunlit plate and weather deck onto the craft ray. Leave the beacon, ring, and label on the buoy — the beacon is 10 km tall and fills the recovery drone. HUD altitude stays the trajectory sample (~0.05 km) and will not show this miss. Check: `?t=1:08:00`, `setCamera("drone")`, HUD off; the hull bottom should meet blue water. A free camera dropped under the plate looks into a black void (the 50 m shell sits above the globe) — that is not a missing ship.

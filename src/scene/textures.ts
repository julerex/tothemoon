...
/** One soft cyclonic cloud cell. */
function paintCloudCell(ctx: CanvasRenderingContext2D, w: number, h: number): void {
  const lon = -180 + Math.random() * 360;
  const lat = -55 + Math.random() * 110;
  const [x, y] = lonLatToXy(lon, lat, w, h);
  const rx = (0.035 + Math.random() * 0.13) * w;
  const ry = (0.012 + Math.random() * 0.055) * h;
  fillCloudGradient(ctx, x, y, rx, ry);
}

/** Color stops for one cloud cell radial gradient. */
function cloudGradientStops(g: CanvasGradient, a: number): void {
...
/** Radial white cloud gradient ellipse. */
function fillCloudGradient(
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  rx: number,
  ...
  const g = ctx.createRadialGradient(x, y, 0, x, y, Math.max(rx, ry));
  cloudGradientStops(g, 0.22 + Math.random() * 0.55);
  ctx.fillStyle = g;
  ctx.beginPath();
...
/** Dense soft cyclonic / cellular decks. */
function paintCloudCells(ctx: CanvasRenderingContext2D, w: number, h: number): void {
  for (let i = 0; i < 110; i++) paintCloudCell(ctx, w, h);
}

/** Brighter cloud tops (sun-catching highlights for LEO). */
function paintCloudHighlights(ctx: CanvasRenderingContext2D, w: number, h: number): void {
  for (let i = 0; i < 35; i++) {
    softBlob(ctx, w, h, -180 + Math.random() * 360, -40 + Math.random() * 80,
      3 + Math.random() * 10, 1.5 + Math.random() * 5,
      `rgba(255,255,255,${0.15 + Math.random() * 0.25})`);
  }
}

/** One ITCZ band strip. */
function paintItczBand(ctx: CanvasRenderingContext2D, w: number, h: number): void {
...
/** ITCZ / band hints. */
function paintItczBands(ctx: CanvasRenderingContext2D, w: number, h: number): void {
  for (let i = 0; i < 10; i++) paintItczBand(ctx, w, h);
}
...
export function makeEarthCloudTexture(size = 1024): HTMLCanvasElement {
  ...
  paintCloudCells(ctx, w, h);
  paintCloudHighlights(ctx, w, h);
  paintItczBands(ctx, w, h);
  return canvas;
}

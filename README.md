...
The craft mesh is a near-true-size Super Heavy + Starship stack (tens of meters): ring welds, denser heat-shield tiles, Raptor field, multi-layer additive plumes, exhaust light, maximum dynamic pressure condensation, and hot-staging dual plumes. The booster stages off at low Earth orbit insert with fallaway + flash, a dim amber free-flyer locator (~30 s), and a brief boostback ignition flash; plumes scale with thrust. Starbase pad (flame trench, deluge steam, chopsticks silhouette) and ascent ground track sit on the spinning Earth. Earth gets a procedural **night-lights** emissive map, stronger atmospheric limb, and soft anti-sun fill; the Moon gets dim **Earthshine**. Lunar landing site beacon + dust puff mark touchdown. In system views the vehicle is tiny — a **red marker** shows its location. Use the **Ship** camera to see liftoff up close.
